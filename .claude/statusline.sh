...
if [ -n "$remaining_pct" ]; then
  ...
  # Build token count string if available (show tokens remaining, not used)
  token_str=""
  if [ -n "$total_input_tokens" ] && [ -n "$context_window_size" ]; then
    remaining_tokens=$(( context_window_size - total_input_tokens ))
    if [ "$remaining_tokens" -lt 0 ]; then remaining_tokens=0; fi
    token_str=" ($(fmt_tokens "$remaining_tokens")/$(fmt_tokens "$context_window_size"))"
  fi
