...
/**
 * Check whether a skill referenced in skill-rules.json actually exists on disk.
 * Guards against config drift: a trigger entry with no `<name>/SKILL.md` would
 * otherwise emit an activation instruction for a skill that cannot be activated.
 * @param {string} name - Skill name (expected to match the skill folder name)
 * @returns {boolean} True if `.claude/skills/<name>/SKILL.md` exists
 */
...
function extractFilePaths(prompt) {
  ...
  // Match explicit paths with Python/engineering extensions
  ...
  // Match paths starting with common engineering directories
  const dirPattern =
  ...
  for (const match of matches) {
    ...
    if (skill?.relatedSkills) {
      for (const relatedName of skill.relatedSkills) {
        if (!matchedNames.has(relatedName) && skillExists(relatedName)) {
...
function evaluate(prompt) {
  ...
  for (const [name, skill] of Object.entries(skills)) {
    ...
    if (match && match.score >= config.minConfidenceScore && skillExists(name)) {
      matches.push(match);
    }
  }
